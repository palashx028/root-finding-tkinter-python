...
def false_position_method(func_str, error_tol=1e-4):
    x = symbols('x')
...
def plot_function(expr, root):
    ...
    plt.title('Function Plot with Root (False Position Method)')
    plt.xlabel('x')
    plt.ylabel('f(x)')
    plt.legend()
    plt.grid(True)
...
def run_false_position():
    ...
    try:
        expr, iterations, root, froot = false_position_method(func_str, error_tol)
...
root = tk.Tk()
root.title("False Position Method Solver")
...
entry_func.insert(0, "x^3 - 2*x^2 - 1")  # default example
...
btn_run = tk.Button(root, text="Run False Position Method", command=run_false_position)
...
root.mainloop()
# This code implements a GUI for the False Position Method to find roots of functions.
